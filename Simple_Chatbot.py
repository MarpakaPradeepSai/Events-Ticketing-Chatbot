...
# Caching helps avoid re-loading the model on every interaction
@st.cache(allow_output_mutation=True)
def load_model_and_tokenizer():
    # Use the GitHub URL where your model is stored
...
def load_spacy_model():
    # Use a smaller model for speed. (Change to "en_core_web_trf" if you prefer and have the resources.)
...
def replace_placeholders(response, dynamic_placeholders, static_placeholders):
    # Replace both static and dynamic placeholders in the response text
...
def extract_dynamic_placeholders(user_question):
    ...
    for ent in doc.ents:
        if ent.label_ == "EVENT":  # Customize this label if needed
            event_text = ent.text.title()
            dynamic_placeholders['{{EVENT}}'] = f"<b>{event_text}</b>"
        elif ent.label_ == "GPE":  # GPE for geopolitical entity (city names)
...
st.write("Ask your ticketing-related question below:")
