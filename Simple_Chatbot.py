...
# Load the transformer-based SpaCy model (download if not already installed)
try:
...
# Path to your fine-tuned model folder (make sure ALBERT_Model folder is in your repo)
...
# Category labels mapping (as defined in your original code)
category_labels = {
...
# Static placeholders used in the responses
static_placeholders = {
...
# Function to extract dynamic placeholders (e.g., event and city) from the user question using SpaCy NER
def extract_dynamic_placeholders(user_question):
    doc = nlp(user_question)
    dynamic_placeholders = {}
    for ent in doc.ents:
        if ent.label_ == "EVENT":  # Adjust label if your custom NER uses a different one for events
...
st.write("Ask your question below and the chatbot will respond accordingly.")
...
if st.button("Submit"):
    if user_question:
        ...
        # Get the corresponding response template
        ...
        st.markdown(f"**Predicted Category:** {predicted_category_name}")
