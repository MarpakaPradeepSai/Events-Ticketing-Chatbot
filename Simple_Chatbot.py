...
# --- Dropdown and Button section (Horizontally Aligned) ---
col1, col2 = st.columns([3, 1])  # Create two columns: 3/4 width for selectbox, 1/4 for button
...
with col1:
    selected_query = st.selectbox(
        ...
        key="query_selectbox",
        label_visibility="collapsed" # Hide the label text itself, rely on placeholder text or surrounding text
    )

with col2:
    # Using st.markdown to inject CSS for vertical alignment of the button within its column
    st.markdown("""
        <style>
            /* Target the specific button using its key within the column */
            div[data-testid="stVerticalBlock"] div[data-testid="stButton"] button[kind="secondary"] {
                margin-top: 28px; /* Adjust this value as needed for perfect alignment */
            }
        </style>
    """, unsafe_allow_html=True)
    process_query_button = st.button("Ask this question", key="query_button", use_container_width=True) # Use full column width
